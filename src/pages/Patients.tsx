...
export default function Patients(): React.ReactElement {
  ...
  const save = async () => {
    const docRef = doc(db, "/patients/", `${selected}`);
    await deleteDoc(docRef);
    const queryRef = query(
      PressCollecationRef,
      where('patient','==',selected)
    );
    const deletDocs = await getDocs(queryRef);
    await Promise.all(
      deletDocs.docs.map(async (DocEelemnt)  =>  {
        const docToDelete = doc(db,'/presciptions/',DocEelemnt.id);
  ...
  return (
    <>
      <Box sx={{ display: "flex", minHeight: "100vh" }}>
        ...
        >
          <Header onDrawerToggle={handleDrawerToggle} title={"Patients List"} />
          <Box
            component="main"
            flex={1}
            overflow="auto"
            ...
            <Paper sx={{ maxWidth: "95%", margin: "auto", padding: "15px" }}>
              {loader ? (
                ...
                >
                  ...
                  <DialogActions>
                    <Button onClick={handleClose}>Cenecl</Button>
                    <Button
                      onClick={async () => {
                        setLoader(true);
                        handleClose();
                                ...
                                <>
                                  ...
                                  >
                                    ...
                                    <Typography
                                          ...
                                          color="text.info"
                                        >
                                          {doctor.data.nationalNumber}
                                        </Typography>
                                    </TableCell>
                                    <TableCell align="left">
                                    <Button onClick={()=> handleClickOpen(`${doctor.id}`)} variant="contained" size="small" color="error" >
                                        Delete
                                      </Button>
                                    </TableCell>
                                  </TableRow>
